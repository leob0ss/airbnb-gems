...
async function scrapePage(cursor) {
  ...
  let data;
  try { data = JSON.parse(match[1]); } catch { return { listings: [], nextCursor: null }; }

  const staysEntry = (data?.niobeClientData ?? []).find(([k]) => k?.startsWith("StaysSearch"));
  if (!staysEntry) return { listings: [], nextCursor: null };

  const staysResults = staysEntry[1]?.data?.presentation?.staysSearch?.results ?? {};
  ...
  const listings = results
    ...
    .map((r) => {
      ...
      return {
        id,
        name: r.nameLocalized?.localizedStringWithTranslationPreference ?? r.subtitle ?? "",
        cardTitle: r.title ?? "",
...
async function fetchListingDetails(listingId) {
  ...
  } catch { return { description: "", propertyType: "" }; }
}
...
- LOW: not a ${KEYWORD} (just a cabin/house near trees, or uses the word loosely)`;
  ...
  try {
    ...
    const answer = json?.choices?.[0]?.message?.content?.trim().toUpperCase() ?? "LOW";
    return answer.startsWith("HIGH") ? "HIGH" : answer.startsWith("MEDIUM") ? "MEDIUM" : "LOW";
  } catch { return "LOW"; }
}
...
for (let i = 0; i < MAX_PAGES; i++) {
  ...
  if (listings.length === 0) { log("No listings found, stopping."); break; }

  let pageHighs = 0;
  for (const listing of listings) {
    if (totalRead >= MAX_LISTINGS_READ) { log(`🛑 Reached ${MAX_LISTINGS_READ} listings read — stopping.`); break; }
    totalRead++;
    await sleep(DELAY_MS);
    const score = await scoreListing(listing);
    log(`  ${score === "HIGH" ? "✅" : score === "MEDIUM" ? "🟡" : "❌"} [${score}] ${listing.name.slice(0, 55)} — ${listing.city ?? "?"}`);
    if (score === "HIGH") { highListings.push(listing); pageHighs++; }
  }
  log(`  → ${pageHighs} HIGHs on page ${pageNum} (total: ${highListings.length})\n`);

  if (totalRead >= MAX_LISTINGS_READ) { log(`🛑 Reached ${MAX_LISTINGS_READ} listings read — stopping.`); break; }
  if (highListings.length >= MAX_HIGHS) { log(`🎯 Reached ${MAX_HIGHS} HIGH listings — stopping early.`); break; }
  if (!nextCursor) { log("No more pages."); break; }
  cursor = nextCursor;
  await sleep(DELAY_MS);
}
